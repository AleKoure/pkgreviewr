...
![pkgreviewr hex sticker](reference/figures/hex-sticker.svg)

`pkgreviewr` reviews R packages with a section-based workflow. It
gathers deterministic QA signals locally, builds targeted context for
each review section, asks an LLM for concise section outputs,
synthesizes an overall diagnostic from section summaries, and can
...
- [`collect_review_data()`](https://alekoure.github.io/pkgreviewr/reference/collect_review_data.md)
  for deterministic signal collection
...
Use a custom backend function when you want to manage provider
...
Use an `ellmer` chat object when you want a ready-made provider client.
The `ellmer` reference for constructing chat objects is at
...
The returned object is a `pkgreviewr_report` character vector with
attributes for draft output, section results, traces, provenance, and
...
An example generated report is available at
[test_review.md](https://alekoure.github.io/pkgreviewr/test_review.md).
